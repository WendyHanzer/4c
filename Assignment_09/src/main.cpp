...
///////////////////////////////////////
	...
	//***create the sphere rigid body***
        ...
        sphereRigidBody->setFriction(0.0);
	...
	//***create the P2 rigid body***
        ...
        cylinderRigidBody->setFriction(0.0);
...
void render()
{
	...
	for(int meshindex =0; meshindex< rink[0].numMesh; meshindex++){
		glBindBuffer(GL_ARRAY_BUFFER, rink[meshindex].vbo_Geo);
		...
		glVertexAttribPointer( loc_tex,
		                       ...
		                       (void*)offsetof(Vertex,texuv));
		//draw board obj
		rink[meshindex].Texs->Bind(GL_TEXTURE0);
		glDrawArrays(GL_TRIANGLES, 0, rink[meshindex].NumVert);//mode, starting index, count
	}
    //clean up
...
Object *modelLoader(const char *objName)
    ...
    glBufferData(GL_ARRAY_BUFFER, output[meshindex].NumVert*20,
